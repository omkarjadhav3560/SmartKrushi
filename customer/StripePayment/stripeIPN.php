...
$dbname="agriculture_portal";
	...
	// Check if userlogin is email or phone number
	if (filter_var($userlogin, FILTER_VALIDATE_EMAIL)) {
		$customerquery="SELECT * from custlogin where email='".$userlogin."'";
	} else {
		$customerquery="SELECT * from custlogin where phone_no='".$userlogin."'";
	}
	
	$customerresult=mysqli_query($conn,$customerquery);
	$row=mysqli_fetch_array($customerresult);

	// Check if row exists before accessing array elements
